...
static void ext_session_buffer_size(void *data,
	...
	cast->pending_constraints.dirty |= XDPW_BUFFER_CONSTRAINTS_DIMENSIONS;
}
...
static void ext_session_shm_format(void *data,
	...
	fmt->stride = 0;
	cast->pending_constraints.dirty |= XDPW_BUFFER_CONSTRAINTS_SHM_FORMATS;
...
static void ext_session_dmabuf_device(void *data,
	...
	cast->pending_constraints.dirty |= XDPW_BUFFER_CONSTRAINTS_DEVICE;
...
static void ext_session_dmabuf_format(void *data,
	...
	cast->pending_constraints.dirty |= XDPW_BUFFER_CONSTRAINTS_DMABUF_FORMATS;
	free(fmt_name);
}
...
static void ext_frame_failed(void *data,
	...
	case EXT_IMAGE_COPY_CAPTURE_FRAME_V1_FAILURE_REASON_STOPPED:
		logprint(ERROR, "ext: frame capture failed: capture session stopped");
		break;
	default:
		abort();
	}
